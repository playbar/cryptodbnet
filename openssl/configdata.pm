...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/usr/local/openssl no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "darwin-thread-multi-2level",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.18.2",
  perlargv => [ "darwin64-x86_64-cc", "--prefix=/usr/local/openssl" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
